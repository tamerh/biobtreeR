...
```{r}
bbBuildData(datasets = "taxonomy,hgnc,uniprot,go",targetDatasets = "hgnc,uniprot,taxonomy,ufeature,go,pdb,taxchild,taxparent") 
```
...
```{r}
bbMapFilter("msh6,stk11,bmpr1a,smad4,brca2","map(uniprot).filter(uniprot.reviewed)",source ="hgnc")
```
...
Rich gene centric usecase requires to build Ensembl data for instance Ensembl human data is built with following way
...
Examples usage of building other Ensembl genomes 
```{r eval=FALSE}
