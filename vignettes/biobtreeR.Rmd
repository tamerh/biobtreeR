...
**Map proteins in human to interpro identifiers with conserved site type**

```{r}
res<-bbMapFilter("homo sapiens",'map(uniprot).map(interpro).filter(interpro.type=="Conserved_site")',attrs = "type,short_name")
head(res)
```

**Get all variation identifiers in human with given condition**

```{r}
res<-bbMapFilter("homo sapiens",'map(uniprot).map(ufeature).filter(ufeature.original=="I" && ufeature.variation=="S").map(variantid)')
head(res)
```
