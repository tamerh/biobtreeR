...
## Build data
biobtreeR process and store the datasets locally by fetching datasets from their public locations. To build the datasets `bbBuildData` is used. And once this process is completed it does not require to perform again unless to work on new datasets or fetching the latest version of the datasets.
...
Examples for building with real datasets 
```{r eval=FALSE}
...
Once the building data completed web server is started with `bbStart` which allows performing queries and provide web interface data exploration.
...
## List available datasets and Ensembl genomes 

In order to use in the data build process availabe datasets and Ensembl genomes are listed in following way
...
```{r message=FALSE}
head(bbListGenomes("ensembl_bacteria"))
```
...
Note that there are several other gene related datasets without attributes and can be used in mapping queries such as probesets, genebank and entrez etc. Full dataset list can be discovered with `bbListDataset`. Now lets build example mapping queries,
...
```{r}
res<-bbMapping("ATP5MC3,TP53",'map(transcript).map(exon)',attrs = "seq_region_name")
head(res)
```
...
```{r}
res<-bbMapping("shh",'map(ortholog)')
head(res)
```
...
```{r}
bbMapping("fry",'map(paralog)',showInputColumn = TRUE)
```
...
```{r eval = FALSE}
res<-bbMapping("homo_sapiens",'map(ensembl).filter(ensembl.seq_region_name=="Y" && ensembl.biotype=="lncRNA")',attrs = "name")
head(res)
```
